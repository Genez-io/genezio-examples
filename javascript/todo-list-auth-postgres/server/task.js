...
import { GenezioAuth, GenezioDeploy } from "@genezio/types";
...
export class TaskService {
  ...
  async #generateUniqueId() {
    const maxId = await TaskModel.max("taskId");
    if (maxId == null) {
      return 0;
    }
    return maxId + 1;
   ...
   *
   * @param {*} gnzContext The genezio context for the authentification session.
   * @returns An object containing two properties: { success: true, tasks: tasks }
   */
  @GenezioAuth()
  async getAllTasks(gnzContext) {
    if (!process.env.POSTGRES_URL) {
      console.log(red_color, missing_env_error);
      return { success: false, tasks: [], err: missing_env_error };
    }
    const ownerId = gnzContext.user?.userId;
    if (!ownerId)
      return {
        success: false,
        tasks: [],
    ...
    console.log(
      `Get all tasks by user request received with userID ${gnzContext.user?.userId}`
    );
    let tasks;
    try {
   ...
   */
  @GenezioAuth()
  async createTask(gnzContext, title) {
    if (!process.env.POSTGRES_URL) {
      console.log(red_color, missing_env_error);
   ...
   */
  @GenezioAuth()
  async updateTask(gnzContext, id, title, solved) {
    if (!process.env.POSTGRES_URL) {
      console.log(red_color, missing_env_error);
      return { success: false, err: missing_env_error };
    }
   ...
   */
  @GenezioAuth()
  async deleteTask(gnzContext, id) {
    if (!process.env.POSTGRES_URL) {
      console.log(red_color, missing_env_error);
      return { success: false, err: missing_env_error };
    }
