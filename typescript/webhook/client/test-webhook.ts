...
(async () => {
  ...
  if (url.length === 0) {
    console.error(
      'Replace the "url" variable in test-webhook-example.js file with the base URL that was generated by the "genezio deploy" or "genezio local" command.',
    );
    return;
  }
  ...
  let response = await axios.get(
    url + "/HelloWorldHttpExample/handleQueryParams?name=john",
  );
  console.log(response.data);

  // Send the request with plain text
  response = await axios.post(
    url + "/HelloWorldHttpExample/handleSimpleTextRequest",
    "text in body",
  ...
  response = await axios.post(
    url + "/HelloWorldHttpExample/handleMultipartData",
    formData,
    {
