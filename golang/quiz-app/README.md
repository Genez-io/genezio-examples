...
This is a simple project with a server and a client for a quiz app. The server is built with [Go](https://go.dev/) and [Prisma](https://www.prisma.io/). The client is built with [React](https://reactjs.org/).
...
1. Get a Mysql connection URL. Follow this [tutorial](https://dev.mysql.com/doc/mysql-getting-started/en/) to learn how to create a MySQL database.
2. Create a `server/.env` file and add the following environment variables:

```env
MYSQL_DB_URL=<your-mysql_db_url>
```
...
```
cd ./genezio-examples/golang/quiz-app
```
...
Deploy your project using the genezio deploy command from the `./genezio-examples/golang/quiz-app` directory.
